...
for frame_input=300:500
%% Determinnation whether 
Blending_update_flag=0;
RPY_update_flag=0; 
    if Blending_update_flag==1
       ...
       Nona_Generator_inherited_Rv(Date_str,frame_input);
       % Step 4
       % Copy mask
       frame_mask_input=frame_input-1;
       Mask_Copy_Rv(frame_input,frame_mask_input);
       % Step 5
       % Produce Mask via enblend
       frame_mask_load=frame_input;
       Mask_load_inherited_Rv(frame_input,frame_mask_load);
       % Pass Camera Poses into next frame
       RPY_Passer_inherited_Rv(Date_str,frame_input+1,frame_input);
    end
%% Step 1
